...
#mz    delz    = -1*nc_file['delz'][0,::-1,i,j] #derive zh
    dpres   = nc_file['dpres'][0,::-1,i,j]
...
#mz: calculate delz in hydrostatic equilibrium
    ...
    for k in range(nlevs):
       ...
       delz[k] = -1*dpres[k]/(grav*rho[k])

#mz    delz = np.asarray(delz)
    zh = np.zeros(nlevs)
    zh[0] = 0.5*delz[0]
...
def get_UFS_forcing_data_advective_tendency(dir, i, j, tile, neighbors, dx, dy, nlevs, lam, save_comp_data, vertical_method, geos_wind_forcing, wind_nudge, geo_pts, geo_neighbors, geo_dx, geo_dy, use_actual_geos_wind):
    ...
    omga   = np.zeros((n_filesA,nlevs,npts,npts))
    pfull  = np.zeros((n_filesA,nlevs))
    rho    = np.zeros((n_filesA,nlevs,npts,npts))
...
#mz: under hydrostatic equilibrium: omega=-rho*grav*dzdt
                ...
                rho[count-1,:,ii,jj] = pfull[count-1,:]/(rdgas*temp[count-1,:,ii,jj])
                w_wind[count-1,:,ii,jj] =  -1.0*omga[count-1,:,ii,jj]/(grav*rho[count-1,:,ii,jj])
#mz
